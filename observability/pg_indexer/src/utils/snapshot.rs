...
pub struct Snapshot {
    ...
    pub marginfi_accounts: HashMap<Pubkey, WrapperMarginfiAccount>,
    pub price_feeds: HashMap<Pubkey, OracleData>,
}

/// Wraps a MarginfiAccount
/// Adds a update time to ease comparison when posting to db
#[derive(Clone, Debug)]
pub struct WrapperMarginfiAccount {
    pub account: MarginfiAccount,
    // Last time this account was updated
    pub update_time: DateTime<Utc>,
    // Whether this account has been pushed to the db or not
    pub pushed: bool,
}
...
impl Snapshot {
    ...
    pub async fn create_entry(&mut self, account_pubkey: &Pubkey, account: &Account) {
        // Pyusd bank is breaking the snapshot
        //if account_pubkey == &Pubkey::from_str("8UEiPmgZHXXEDrqLS3oiTxQxTbeYTtPbeMBxAd2XGbpu").unwrap() {
        //    return;
        //}
        ...
        match routing_info {
            ...
            AccountRoutingType::MarginfiAccount => {
                self.marginfi_accounts.insert(
                    *account_pubkey,
                    WrapperMarginfiAccount {
                        account: MarginfiAccount::try_deserialize(&mut (&account.data as &[u8]))
                            .unwrap(),
                        update_time: Utc::now(),
                        pushed: false,
                    },
