...
use crate::prelude::{
    get_emissions_authority_address, get_emissions_token_account_address, load_and_deserialize2,
    MintFixture, TestContext, TokenAccountFixture,
};
use anchor_lang::{
...
impl BankFixture {
    pub fn new(ctx: Rc<RefCell<TestContext>>, key: Pubkey, mint_fixture: &MintFixture) -> Self {
        Self {
            ctx,
            key,
    ...
    pub async fn get_price(&self) -> f64 {
        ...
        let mut oracle_account = self.ctx.borrow_mut().svm.get_account(&oracle_key).unwrap();
    ...
    pub async fn load(&self) -> Bank {
        load_and_deserialize2::<Bank>(self.ctx.clone(), &self.key).await
    }
    ...
    pub async fn update_config(&self, config: BankConfigOpt) -> anyhow::Result<()> {
        ...
        let tx = Transaction::new_signed_with_payer(
            ...
            self.ctx.borrow().svm.latest_blockhash(),
        );

        self.ctx.borrow_mut().svm.send_transaction(tx)?;
