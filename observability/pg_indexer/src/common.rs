...
pub fn pyth_price_to_fixed(price_feed: &PriceFeed) -> anyhow::Result<I80F48> {
    ...
    let price = match exponent.cmp(&0) {
        std::cmp::Ordering::Equal => price,
        std::cmp::Ordering::Less => price.checked_div(scaling_factor).unwrap(),
        std::cmp::Ordering::Greater => price.checked_mul(scaling_factor).unwrap(),
    };
