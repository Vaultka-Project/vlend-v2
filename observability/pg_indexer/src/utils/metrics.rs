...
impl MarginfiGroupMetrics {
    pub fn new(
        ...
        let group_marginfi_accounts_iter = snapshot
            .marginfi_accounts
            .iter()
            .filter(|(_, marginfi_account)| marginfi_account.account.group.eq(marginfi_group_pk));
    ...
    pub fn to_row(&self) -> MarginfiGroupMetricsRow {
        MarginfiGroupMetricsRow {
            ...
            timestamp: DateTime::from_timestamp(self.timestamp, 0)
                .unwrap_or_else(|| Utc.timestamp_opt(self.timestamp, 0).unwrap()),
...
impl LendingPoolBankMetrics {
    pub fn new(
        ...
        let lenders_count = snapshot
            ...
            .iter()
            .filter(|(_, account)| {
                account.account.lending_account.balances.iter().any(|a| {
                    a.active
