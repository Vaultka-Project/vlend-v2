...
use crate::prelude::{MintFixture, TestContext};
use crate::utils::*;
...
pub struct MarginfiGroupFixture2 {
    ctx: Rc<RefCell<TestContext>>,
    pub key: Pubkey,
}

impl MarginfiGroupFixture2 {
    pub async fn new(ctx: Rc<RefCell<TestContext>>, config: GroupConfig) -> MarginfiGroupFixture2 {
    ...
    ) -> Result<BankFixture, FailedTransactionMetadata> {
    ...
    pub async fn try_lending_pool_add_bank_with_seed(
        ...
        self.ctx.borrow_mut().svm.se(tx).await?;
