use crate::{prelude::TestContext, transfer_hook::TEST_HOOK_ID, ui_to_native};
...
#[derive(Clone)]
pub struct MintFixture2 {
    pub ctx: Rc<RefCell<TestContext>>,
    pub key: Pubkey,
...
impl MintFixture2 {
    pub async fn new(
        ...
        let mint = {
            ...
            let minimum_balance = ctx.svm.minimum_balance_for_rent_exemption(Mint::LEN);
            ctx.svm.airdrop(&from, minimum_balance).unwrap();
            ...
            let mint_account = ctx.svm.get_account(&keypair.pubkey()).unwrap();
    ...
    pub async fn new_token_22(
        ...
        let mint = {
            ...
            let extension_types = SupportedExtension2::types(extensions.iter());
            let len = ExtensionType::try_calculate_account_len::<spl_token_2022::state::Mint>(
                &extension_types,
            )
            .unwrap();
            let minimum_balance = ctx.svm.minimum_balance_for_rent_exemption(len);
            ...
            if extensions.contains(&SupportedExtension2::TransferHook) {
            ...
            if extensions.contains(&SupportedExtension2::TransferHook) {
                ctx.svm.get_account(&extra_metas_address).unwrap();
            }
    ...
    pub fn new_from_file(ctx: &Rc<RefCell<TestContext>>, relative_path: &str) -> MintFixture2 {
        let ctx_ref = Rc::clone(ctx);
        ...
        let (address, account_info) = {
            ...
            ctx.svm.set_account(address, account_info.clone().into());
    ...
    pub async fn reload(&mut self) {
        let mint_account = self.ctx.borrow_mut().svm.get_account(&self.key).unwrap();
        self.mint =
            StateWithExtensionsOwned::<spl_token_2022::state::Mint>::unpack(mint_account.data)
                .unwrap()
                .base;
    ...
    pub async fn mint_to<T: Into<f64>>(&mut self, dest: &Pubkey, ui_amount: T) {
        ...
        self.ctx.borrow_mut().svm.send_transaction(tx).unwrap();
    ...
    pub async fn create_empty_token_account(&self) -> TokenAccountFixture2 {
    ...
    pub async fn create_token_account_and_mint_to<T: Into<f64>>(
        &self,
        ui_amount: T,
    ) -> TokenAccountFixture2 {
        let payer = self.ctx.borrow().payer.pubkey();
        let token_account_f = TokenAccountFixture2::new_with_token_program(
            self.ctx.clone(),
            &self.key,
            &payer,
            &self.token_program,
...
pub struct TokenAccountFixture2 {
    ctx: Rc<RefCell<TestContext>>,
    pub key: Pubkey,
...
impl TokenAccountFixture2 {
    pub async fn create_ixs(
        ...
        // 1) Fetch mint
        let mint_account = ctx.borrow_mut().svm.get_account(mint_pk).unwrap();
        let mint_exts =
    ...
    pub async fn new_with_keypair(
...

        {
            ...
            let rent = ctx.borrow_mut().svm.get_sysvar::<Rent>();
            ...
            let tx = Transaction::new_signed_with_payer(
                ...
                ctx.borrow().svm.latest_blockhash(),
            );

            ctx.borrow_mut().svm.send_transaction(tx).unwrap();
        }

        let mut ctx = ctx.borrow_mut();
        let account = ctx.svm.get_account(&keypair.pubkey()).unwrap();
    ...
    pub async fn new(
        ctx: Rc<RefCell<TestContext>>,
        mint_fixture: &MintFixture2,
        owner_pk: &Pubkey,
    ) -> TokenAccountFixture2 {
        let keypair = Keypair::new();
        let mint_pk = mint_fixture.key;
        TokenAccountFixture2::new_with_keypair(
            ctx,
            &mint_pk,
            owner_pk,
            &keypair,
    ...
    pub async fn new_with_token_program(
        ...
        let keypair = Keypair::new();
        TokenAccountFixture2::new_with_keypair(ctx, mint_pk, owner_pk, &keypair, token_program)
            .await
    }

    pub async fn fetch(ctx: Rc<RefCell<TestContext>>, address: Pubkey) -> TokenAccountFixture2 {
...
pub async fn balance_of(ctx: Rc<RefCell<TestContext>>, pubkey: Pubkey) -> u64 {
...
impl SupportedExtension2 {
    pub fn instruction(&self, mint: &Pubkey, key: &Pubkey) -> Instruction {
        match self {
            SupportedExtension2::MintCloseAuthority => {
            ...
            SupportedExtension2::InterestBearing => {
            ...
            SupportedExtension2::PermanentDelegate => {
    ...
    pub fn space<'a>(exts: impl Iterator<Item = &'a SupportedExtension2>) -> usize {
        exts.map(|e| match e {
            SupportedExtension2::MintCloseAuthority => pod_get_packed_len::<MintCloseAuthority>(),
            SupportedExtension2::InterestBearing => pod_get_packed_len::<InterestBearingConfig>(),
            SupportedExtension2::PermanentDelegate => pod_get_packed_len::<PermanentDelegate>(),
            SupportedExtension2::TransferHook => pod_get_packed_len::<TransferHook>(),
            SupportedExtension2::TransferFee => pod_get_packed_len::<TransferFee>(),
        })
        .sum()
    }

    fn types<'a>(exts: impl Iterator<Item = &'a SupportedExtension2>) -> Vec<ExtensionType> {
        exts.map(|e| match e {
            SupportedExtension2::MintCloseAuthority => ExtensionType::MintCloseAuthority,
            SupportedExtension2::InterestBearing => ExtensionType::InterestBearingConfig,
            SupportedExtension2::PermanentDelegate => ExtensionType::PermanentDelegate,
            SupportedExtension2::TransferHook => ExtensionType::TransferHook,
            SupportedExtension2::TransferFee => ExtensionType::TransferFeeConfig,
        })
        .collect()
    }
}
